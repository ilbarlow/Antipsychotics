...
#make into arrays for classification
X = np.array(mrFeatMatAll.iloc[:,:100])
y = np.array(conds2['class'])
...
for train_index, test_index in sss.split(X, y):
    ...
    del X_train, X_test, y_train, y_test, train_index,test_index
...
#make into a dataframe
feat_scores = pd.DataFrame(data = np.array(temp), columns = mrFeatsA[:10])
del temp
